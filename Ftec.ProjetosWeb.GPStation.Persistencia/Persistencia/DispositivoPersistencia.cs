			...
			try
			{
				using (con)
				{
					using (var comando = new SqlCommand())
					{
						...
						comando.CommandText ="SELECT[Id],[Nome],[Latitude],[Longitude]FROM[dbo].[dispositivo]WHERE[Nome]=@Nome ORDER BY [Nome]";
		...
		public bool Inserir(Dispositivo dispositivo)
		{
			...
			try
			{
				...
				using (var comando = new SqlCommand())
				{
					comando.Transaction = transacao;
					comando.Connection = con;
					comando.CommandText = "INSERT INTO[dbo].[dispositivo] ([Id],[Nome],[Latitude],[Longitude])VALUES(@Id,@Nome,@Latitude,@Longitude);";
					comando.Parameters.AddWithValue("Id", dispositivo.Id);
					comando.Parameters.AddWithValue("Nome", dispositivo.Nome);
